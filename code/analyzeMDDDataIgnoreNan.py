        ...
        if i not in ignoreColumns:
            ...
            if i not in bdiColumns:
                ...
                if len(mddGroup) > 2:
                    ...
                    plt.show()
                    if pVal < desiredPval:
                        plt.savefig(os.path.join('/','home','zjpeters','Documents','otherLabs','agarza','derivatives','noNaN','sigFigures',f'{columnHeadersAndFilenames[i]["filename"]}_pearson_r_{columnHeadersAndFilenames[bdi]["filename"]}.png'))
                    plt.savefig(os.path.join('/','home','zjpeters','Documents','otherLabs','agarza','derivatives','noNaN','BDICorrelations',f'{columnHeadersAndFilenames[i]["filename"]}_pearson_r_{columnHeadersAndFilenames[bdi]["filename"]}.png'))
                    plt.close()
